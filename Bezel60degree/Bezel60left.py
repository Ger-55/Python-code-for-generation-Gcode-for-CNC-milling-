...
def writeinfo():
    print("Writing Information to file BezelLeftInfo.txt ")
    with open("BezelInfo.txt", "w") as Infof:
        Infof.write("File : BezelLeftInfo.txt (in [mm]) \n")
        Infof.write("D material (bezel) depth "+str(format(D, '.2f')+"\n"))
        Infof.write("Radius outer circle      "+str(format(L, '.2f')+"\n"))
        Infof.write("Sphere depth             "+str(format(M, '.2f')+"\n"))
...
with open(GCodefile, "w") as GCodefout:
    ...
    while Z >= -D:
         ...
         while X <= Xe:
              ...
              BGCode = "G0 F1000 X"+Xc+" Y"+yorg+" Z2.00\n"
